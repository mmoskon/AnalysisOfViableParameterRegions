...
class Region: 
	...
	def converged(self):
		if self.components == None or self.prevComponents == None: 
			return False		
...
class Solver:
	...
	def setBoxColors(self, bp, nRegions, ax):
		colors = ["#0E74C8", "#15A357", "r", "k"]
		colorLen = len(colors) 
...
if __name__ == '__main__':
	...
	#flip flop  
	filename =  os.path.join(".", "DFlipFlop", "exper0", "flipflop")   
	print(filename)   
	model = DFlipFlop(parameter_values, np.array(["protein_production", "protein_production", "protein_production", "protein_production", "protein_degradation", "protein_degradation", "Kd", "hill"]), np.array([0.0 , 0.0, 100.0, 0.0]))
	solver = Solver(model)         
	solver.run(filename)              	
